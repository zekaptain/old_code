...
public class SlidingPuzzleState implements SearchState
{
	...
	private int treeDepth; //depth in the search tree
	private SlidingPuzzleState prevState; //a reference to the node that generated this one
	private boolean checkForRepeatedStates; //used to decide if we should repeat previously seen states (on the path to the root) when generating successors
	...
	public SlidingPuzzleState(int w, int h)
	{
		...
		treeDepth = 0; //assume this is brand-new tree
		prevState = null; //no parent for a brand new tree
		checkForRepeatedStates = true;
		initialize();
	}
	...
	public SlidingPuzzleState(SlidingPuzzleState other)
	{
		...
		board = new int[rows][cols];
		treeDepth = other.treeDepth + 1; //this one is one deeper than its parent
		prevState = other; //reference to parent node
		checkForRepeatedStates = other.checkForRepeatedStates;
		for(int r = 0; r < rows; r++)
		{
			for(int c = 0; c < cols; c++)
			{
	...
	/**
	 * Used to tell the a state whether or not repeated states should
	 * be checked for when generating successor states. By default, the
	 * constructors for new trees will set it to true, so you need to 
	 * call this method if you want to change that.
	 * 
	 * @param c - a boolean to set the checkForRepeatedStates field
	 */
	public void setCheckForRepeatedStates(boolean c)
	{
		checkForRepeatedStates = c;
	}
	
	/**
	 * Accessor method for the depth in the search tree.
	 */
	public int getDepth()
	{
		return treeDepth;
	...
	public ArrayList<SearchState> getStateSequence()
	{
		...
		//loop to follow the prevState references all the way back to the root
		SlidingPuzzleState curr = this;
		while(curr != null)
		{
			seq.add(0, curr); //adding to the front so we end up in beginning-to-end order
			curr = curr.prevState;
		}
	...
	public void shuffle(int moves)
	{
		...
		int moveCol;
		boolean madeMove;
		treeDepth = 0; //now it's a new tree
	...
	{
		ArrayList<SearchState> childrenStates = new ArrayList<SearchState>();
		...
		if(blankRow - 1 >= 0) //we can do an up move
		{
			...
			if(checkForRepeatedStates)
			{
				if(!isOnPathToRoot(up))
				{
	...
	/**
	 * Result indicates whether or not the given state is a repeat.
	 * 
	 * @param other - a state that may or may not be a duplicate the sequence found so far
	 * @return a boolean signifying whether or not the param state (other) is 
	 * 		already on the sequence of states between the root and this state
	 */
	public boolean isOnPathToRoot(SlidingPuzzleState other)
	{
		boolean foundDuplicateState = false;
		SlidingPuzzleState curr = this;
		while(curr != null && !foundDuplicateState)
		{
			if(curr.isSame(other))
			{
				foundDuplicateState = true;
			}
			curr = curr.prevState;
		}
		return foundDuplicateState;
	}

	/**
	 * Since the step cost for the sliding puzzle is always 1, the path cost is just the depth in the tree.
	 */
