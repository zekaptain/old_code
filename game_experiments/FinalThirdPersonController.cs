...
/// <summary>
/// INSERT A FUNCTION that can be called by PlayerHealth so that when PlayerHealth hits 0, animations stop besides Idle
/// might have to put this in the animator though
/// </summary>


[RequireComponent(typeof(CharacterController))]

public class FinalThirdPersonController : MonoBehaviour {
	...
	/// <summary>
	/// The attack repeat time.
	/// </summary>
	private float attackRepeatTime = 0.05f;
	private float attackTimeout = 0.15f;
	...
	//are we attacking? (initiated with attack button)
	...
	/// <summary>
	/// ADDED ATTACKING FLOATS
	/// </summary>
	//last time the attack button was clicked down
	private float lastAttackButtonTime = -10.0f;
	...
	void ApplyAttacking () {
		...
		if (lastAttackTime + attackRepeatTime > Time.time)
			return;

		if (canAttack && Time.time < lastAttackButtonTime + attackTimeout) {
	...
	void  Update (){
		...
		// Add attack mode for attacking others
		//if (Input.GetKeyUp (KeyCode.X)) {
		//	Animation.Play("AttackMelee2");
		//}
	}
